...
class Elementor_Widgets_Analyzer {
    ...
    public function add_admin_menu() {
        add_menu_page(
            __('Elementor Widgets Analyzer', 'elementor-widgets-analyzer'),
            __('Widgets Analyzer', 'elementor-widgets-analyzer'),
            'manage_options',
    ...
    public function enqueue_admin_scripts($hook) {
        if ($hook !== 'toplevel_page_elementor-widgets-analyzer') {
            return;
        }
        ...
        wp_localize_script('ewa-admin-js', 'ewa_ajax', array(
            ...
            'strings' => array(
                'analyzing' => __('Analyzing...', 'elementor-widgets-analyzer'),
                'analysis_complete' => __('Analysis complete!', 'elementor-widgets-analyzer'),
                'error' => __('An error occurred.', 'elementor-widgets-analyzer')
            )
        ));
    }
    ...
    public function ajax_analyze_content() {
        ...
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'elementor-widgets-analyzer'));
        }
    ...
    public function ajax_export_data() {
        ...
        switch ($type) {
            ...
            case 'content_types':
                ...
                break;
            default:
                wp_send_json_error(__('Invalid export type.', 'elementor-widgets-analyzer'));
        }
        
        if (empty($data)) {
            wp_send_json_error(__('No data available for export.', 'elementor-widgets-analyzer'));
        }
    ...
    private function export_as_csv($data, $filename) {
        if (empty($data)) {
            wp_send_json_error(__('No data to export.', 'elementor-widgets-analyzer'));
        }
    ...
    public function ajax_get_chart_data() {
        ...
        switch ($chart_type) {
            ...
            case 'top_widgets':
                ...
                break;
            default:
                wp_send_json_error(__('Invalid chart type.', 'elementor-widgets-analyzer'));
        }
    ...
    private function get_widget_usage_chart_data($database, $analyzer) {
        ...
        return array(
            ...
            'datasets' => array(
                array(
                    'label' => __('Widget Usage', 'elementor-widgets-analyzer'),
                    'data' => $data,
    ...
    private function get_content_types_chart_data($database, $analyzer) {
        ...
        return array(
            ...
            'datasets' => array(
                array(
                    'label' => __('Content Count', 'elementor-widgets-analyzer'),
                    'data' => $data,
                    'backgroundColor' => $colors,
                    'borderColor' => $colors,
                    'borderWidth' => 1
    ...
    private function get_widget_distribution_chart_data($database, $analyzer) {
        ...
        if ($other_usage > 0) {
            $labels[] = __('Others', 'elementor-widgets-analyzer');
            $data[] = $other_usage;
            $colors[] = '#cccccc';
        }
        ...
        return array(
            ...
            'datasets' => array(
                array(
                    'label' => __('Widget Distribution', 'elementor-widgets-analyzer'),
                    'data' => $data,
                    'backgroundColor' => $colors,
                    'borderColor' => $colors,
                    'borderWidth' => 1
    ...
    private function get_top_widgets_chart_data($database, $analyzer) {
        ...
        return array(
            ...
            'datasets' => array(
                array(
                    'label' => __('Top Widgets', 'elementor-widgets-analyzer'),
                    'data' => $data,
                    'backgroundColor' => $colors,
                    'borderColor' => $colors,
                    'borderWidth' => 1
